...
playgrid = [str(x) for x in range(1, 10)]
ROW_SEPARATOR = "-----"
ROW_TEMPLATE = "{}|{}|{}"
...
def print_playgrid(grid: list, row_template: str, row_separator: str,
                   winner: bool, message="") -> None:
    """
    Loop and fill template with 3 items extracted from slice of 3 items
    from play grid in each iteration.
    """
    os.system('clear')
    print("Tic Tac Toe")
    for start_value in range(0, 9, 3):
        print(row_separator,
              row_template.format(*grid[start_value:start_value + 3]),
              sep="\n")
    print(row_separator, "\n")
...
def player_is_winner(win_combinations: list, player_turns: dict,
                     current_player_index: int) -> bool:
    """
    Player can win after 3 turns, if less then 2 turns made return False.
    Check if all numbers from any win combination are presented in players
    choices, if yes return True.
    """
...
# def player_is_winner(win_combinations: list, player_turns: dict, current_player_index: int):
#     """
#     Player can win after 3 turns, if less then 2 turns made return False.
...
while not game_winner and len(set(playgrid)) != 2:
    ...
    message = ""
    user_input = input(
        f"Player {PLAYERS[player_index]} turn.\
        Select position on grid to place your mark: ")
    ...
    # Valid input
        ...
        if playgrid[int(user_input) - 1] in PLAYERS:
            ...
            if not (game_winner := player_is_winner(WIN_COMBINATIONS,
                                                    player_choices,
                                                    player_index)):
